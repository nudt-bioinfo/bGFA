...
AssemblyGraph::AssemblyGraph() : m_kmer(0), m_contiguitySearchDone(false),
...
QByteArray AssemblyGraph::getReverseComplement(QByteArray forwardSequence)
{
    ...
    for (int i = forwardSequence.length() - 1; i >= 0; --i)
    {
        ...
        case 'c':
            ...
            break;
        case 'R':
            reverseComplement.append('Y');
            break;
        case 'Y':
            reverseComplement.append('R');
            break;
        case 'S':
            reverseComplement.append('S');
            break;
        case 'W':
            reverseComplement.append('W');
            break;
        case 'K':
            reverseComplement.append('M');
            break;
        case 'M':
            reverseComplement.append('K');
            break;
        case 'r':
            reverseComplement.append('y');
            break;
        ...
        case 'm':
            ...
            break;
        case 'B':
            reverseComplement.append('V');
            break;
        case 'D':
            reverseComplement.append('H');
            break;
        case 'H':
            reverseComplement.append('D');
            break;
        case 'V':
            reverseComplement.append('B');
            break;
        case 'b':
            reverseComplement.append('v');
            break;
...
template <typename T>
double AssemblyGraph::getValueUsingFractionalIndex(std::vector<T> *v, double index) const
{
    if (v->size() == 0)
        return 0.0;
    if (v->size() == 1)
...
GraphFileType AssemblyGraph::getGraphFileTypeFromFile(QString fullFileName)
{
    ...
    if (checkFileIsAsqg(fullFileName))
        return ASQG;
    // Binary GFA (bGFA) files are detected by extension.
    QFileInfo fi(fullFileName);
    if (fi.suffix().toLower() == QString("bgfa"))
        return BGFA;
...
// Build graph from bGFA (binary GFA) using bgfa_graph.hpp
void AssemblyGraph::buildDeBruijnGraphFromBgfa(QString fullFileName, bool *unsupportedCigar, bool *customLabels, bool *customColours, QString *bandageOptionsError)
{
    m_graphFileType = BGFA;
    m_filename = fullFileName;
    ...
    *bandageOptionsError = "";
    ...
    // Load bGFA via helper library
    BGFA_GFA bgfa(fullFileName.toStdString());

    {
        ...
        // Create nodes from segments. Use segment id as the base name.
        const auto &segments = bgfa.getAllSegments();

        // Create edges from LinksSet
        const auto &linksSet = bgfa.getLinksSet();
        const auto &allLinks = linksSet.getAllLinks();
        {
            QApplication::processEvents();

            for (const auto &seg : segments)
            {
                // Name as numeric id string
                QString baseName = QString::fromStdString(std::to_string(seg.getId()));
                QString posName = baseName + "+";
                if (m_deBruijnGraphNodes.contains(posName))
                    throw "load error";

                int length = seg.getLength();

                QByteArray sequence = QString::fromStdString(seg.getSequenceAsString()).toLocal8Bit();
                // If sequence empty in bGFA, keep empty but preserve length
                double depth = 1.0;

                posName = simplifyCanuNodeName(posName);

                DeBruijnNode *node = new DeBruijnNode(posName, depth, sequence, length);
                m_deBruijnGraphNodes.insert(posName, node);
            }

            for (size_t fromId = 0; fromId < allLinks.size(); ++fromId)
            {
                const auto &outSet = allLinks[fromId];
                for (const auto &encoded : outSet)
                {
                    // Encoding format: (to_id << 2) | dir (2-bit)
                    auto toId = static_cast<size_t>(encoded >> 2);
                    unsigned dir = static_cast<unsigned>(encoded & 0x3);
                    // Decode 2-bit orientation: bit1 = from_dir, bit0 = to_dir ("-"=0, "+"=1)
                    bool fromPlus = ((dir >> 1) & 0x1) != 0;
                    bool toPlus = (dir & 0x1) != 0;

                    QString startingNode = QString::fromStdString(std::to_string(fromId)) + (fromPlus ? "+" : "-");
                    QString endingNode = QString::fromStdString(std::to_string(toId)) + (toPlus ? "+" : "-");
                    startingNode = simplifyCanuNodeName(startingNode);
                    endingNode = simplifyCanuNodeName(endingNode);
                    edgeStartingNodeNames.push_back(startingNode);
                    edgeEndingNodeNames.push_back(endingNode);
...
bool AssemblyGraph::loadGraphFromFile(QString filename)
{
    ...
    try
    {
        ...
        if (graphFileType == BGFA)
        {
            bool unsupportedCigar, customLabels, customColours;
            QString bandageOptionsError;
            buildDeBruijnGraphFromBgfa(filename, &unsupportedCigar, &customLabels, &customColours, &bandageOptionsError);
        }
    ...
    if (g_settings->linearLayout)
    {
        ...
        if (successfulIntConversion)
        {
            std::sort(numericallySortedDrawnNodes.begin(), numericallySortedDrawnNodes.end(),
                      [](const QPair<int, DeBruijnNode *> &a, const QPair<int, DeBruijnNode *> &b)
