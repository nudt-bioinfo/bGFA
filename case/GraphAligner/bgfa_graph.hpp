...
public:
    ...
    const std::unordered_set<uword_t> &getLinksFromNode(uword_t index) const
    {
        ...
        {
            std::cout << "index:" << index << " linkSize:" << links_.size() << std::endl;
    ...
    uword_t getLinkToByIndex(uword_t from_id, uword_t i) const
    {
        ...
        {
            std::cout << "from_id:" << from_id << " linkSize:" << links_.size() << std::endl;
            throw std::out_of_range("[Get Link Info] Node index out of range");
        }
        if (i >= links_[from_id].size())
        {
            std::cout << "index:" << i << " linkSize:" << links_[from_id].size() << std::endl;
    ...
    uword_t getLinkDirByIndex(uword_t from_id, uword_t i) const
    {
        ...
        if (i >= links_[from_id].size())
        {
            std::cout << "i:" << i << " linkSize:" << links_.size() << std::endl;
            throw std::out_of_range("[Get Link Info] Link index out of range");
        }
        auto it = links_[from_id].begin();
        std::advance(it, i);
...
public:
    Path() = default;
    explicit Path(const std::vector<uword_t> &nodes) : nodes_(nodes)
    {
        setStoreSize();
    }

    explicit Path(uword_t id, const std::vector<uword_t> &nodes)
        : id_(id), nodes_(nodes)
    {
        setStoreSize();
    }

    explicit Path(uword_t path_id, std::string path_str, LinksSet &links_set, NodeMap &id_to_id) : id_(path_id)
    {
    ...
    explicit Path(std::string walk_str, LinksSet &links_set, NodeMap &id_to_id) : id_(0)
    {
     ...
     */
    std::string toGfaTxt(const LinksSet &links_set) const
    {
        std::ostringstream oss;
        ...
        while (prev_id != (nodes_[1] >> 1))
        {
            if (links_set.getLinksFromNode(prev_id).empty())
                throw std::logic_error(
                    "[Path to GFA Text] No outgoing links from node: " +
...
private:
     ...
     */
    bool isNotUniqueSuccessor(LinksSet &links_set,
...
public:
    ...
    std::string toGfaTxt(const LinksSet & /*links_set*/) const
    {
        ...
        // end marker (kept consistent with Path style)
        // oss << " *";
...
private:
    ...
    void parsePathLine(const std::string &line)
    {
        ...
        Path path(path_num_, path_str, links_, name_to_id_);
    ...
    void parseWalkLine(const std::string &line)
    {
        ...
        Path path(walk_str, links_, name_to_id_);
