...
struct Seeder
{
	...
	Seeder(const AlignerParams &params, const std::unordered_map<std::string, std::vector<SeedHit>> *fileSeeds, const MEMSeeder *memSeeder, const MinimizerSeeder *minimizerSeeder) : mumCount(params.mumCount),
...
void QueueInsertSlowly(moodycamel::ProducerToken &token, moodycamel::ConcurrentQueue<std::string *> &queue, std::string &&str)
{
	...
	{
		if (queue.size_approx() < 100 && queue.enqueue(token, write))
			break;
		std::this_thread::sleep_for(std::chrono::milliseconds(10));
		waited++;
		if (waited >= 10)
		{
			if (queue.enqueue(token, write))
				break;
		}
	}
}
...
void writeCorrectedToQueue(moodycamel::ProducerToken &token, const AlignerParams &params, const std::string &readName, const std::string &original, moodycamel::ConcurrentQueue<std::string *> &correctedOut, const AlignmentResult &alignments)
{
	...
	std::sort(corrections.begin(), corrections.end(), [](const Correction &left, const Correction &right)
...
void filterOutWrongHaplotypeSeeds(std::vector<SeedHit> &seeds, const GraphAlignerCommon<size_t, int64_t, uint64_t>::AlignerGraphsizedState &reusableState)
{
	...
	for (auto t : reusableState.bigraphNodeForbiddenSpans)
	{
		...
		size_t roundedEnd = 0;
		if (std::get<1>(t) > 0)
			roundedStart = (std::get<1>(t) / 64) * 64;
		if (std::get<2>(t) > 0)
			roundedEnd = ((std::get<2>(t) + 63) / 64) * 64;
	...
	for (size_t i = seeds.size() - 1; i < seeds.size(); i--)
	{
		...
		if (forbiddenSpans[bigraphNodeId].size() == 0)
			continue;
...
void runComponentMappings(const AlignmentGraph &alignmentGraph, const DiploidHeuristicSplitter &diploidHeuristic, moodycamel::ConcurrentQueue<std::shared_ptr<FastQ>> &readFastqsQueue, std::atomic<bool> &readStreamingFinished, int threadnum, const Seeder &seeder, AlignerParams params, moodycamel::ConcurrentQueue<std::string *> &GAMOut, moodycamel::ConcurrentQueue<std::string *> &JSONOut, moodycamel::ConcurrentQueue<std::string *> &GAFOut, moodycamel::ConcurrentQueue<std::string *> &correctedOut, moodycamel::ConcurrentQueue<std::string *> &correctedClippedOut, moodycamel::ConcurrentQueue<std::string *> &deallocqueue, AlignmentStats &stats)
{
	...
	while (true)
	{
		...
		while (fastq == nullptr && !readFastqsQueue.try_dequeue(fastq))
		{
			...
			if (!readFastqsQueue.try_dequeue(fastq) && tryBreaking)
				break;
			std::this_thread::sleep_for(std::chrono::milliseconds(10));
		}
		if (fastq == nullptr)
		...
		if (params.hpcCollapse)
			fastq->sequence = hpcCollapse(fastq->sequence);
		...
		try
		{
			if (seeder.mode != Seeder::Mode::None)
			{
				...
				if (params.useDiploidHeuristic)
					filterOutWrongHaplotypeSeeds(seeds, reusableState);
				...
				if (seeds.size() == 0)
				{
					...
					if (params.outputCorrectedFile != "")
						writeCorrectedToQueue(correctedToken, params, fastq->seq_id, fastq->sequence, correctedOut, alignments);
					continue;
				}
				...
				if (processedSeeds.size() > params.maxClusterExtend)
				{
					...
					std::sort(processedSeeds[params.maxClusterExtend - 1].hits.begin(), processedSeeds[params.maxClusterExtend - 1].hits.end(), [](const ProcessedSeedHit &left, const ProcessedSeedHit &right)
							  { return left.seqPos < right.seqPos; });
		...
		std::sort(alignments.alignments.begin(), alignments.alignments.end(), [](const AlignmentResult::AlignmentItem &left, const AlignmentResult::AlignmentItem &right)
				  { return left.alignmentStart < right.alignmentStart; });
		...
		std::sort(alignments.alignments.begin(), alignments.alignments.end(), [](const AlignmentResult::AlignmentItem &left, const AlignmentResult::AlignmentItem &right)
				  { return left.alignmentXScore > right.alignmentXScore; });
		...
		for (size_t i = 0; i < alignments.alignments.size(); i++)
		{
			...
			if (params.outputCorrectedFile != "" || params.outputCorrectedClippedFile != "")
				AddCorrected(alignments.alignments[i]);
		...
		try
		{
			if (params.outputGAMFile != "")
				writeGAMToQueue(GAMToken, params, GAMOut, alignments);
			if (params.outputJSONFile != "")
				writeJSONToQueue(JSONToken, params, JSONOut, alignments);
			if (params.outputGAFFile != "")
				writeGAFToQueue(GAFToken, params, GAFOut, alignments);
			if (params.outputCorrectedFile != "")
				writeCorrectedToQueue(correctedToken, params, fastq->seq_id, fastq->sequence, correctedOut, alignments);
			if (params.outputCorrectedClippedFile != "")
				writeCorrectedClippedToQueue(clippedToken, params, correctedClippedOut, alignments);
		}
		catch (const ThreadReadAssertion::AssertionFailure &a)
		{
			reusableState.clear();
...
AlignmentGraph getGraph(std::string graphFile, MEMSeeder **mxmSeeder, const AlignerParams &params)
{
	...
	try
	{
		...
		else if (graphFile.size() > 5 && graphFile.substr(graphFile.size() - 5) == ".bgfa")
		{
			auto graph = GfaGraph::LoadFromBGFA(graphFile);
			if (loadMxmSeeder)
			{
				std::cout << "Build MUM/MEM seeder from the graph" << std::endl;
				*mxmSeeder = new MEMSeeder{graph, params.seederCachePrefix, params.uniqueMemBonusFactor, params.lowMemoryMEMIndexConstruction, params.MEMindexUsesWaveletTree, params.MEMwindowsize};
...
std::unordered_map<std::string, std::vector<SeedHit>> loadGafSeeds(const AlignmentGraph &alignmentGraph, const std::string &gafFile)
{
	...
	while (file.good())
	{
		...
		{
			while (tabPoses[i] < line.size() && line[tabPoses[i]] != '\t')
				tabPoses[i] += 1;
			if (i < 8)
				tabPoses[i + 1] = tabPoses[i] + 1;
		}
		if (tabPoses[8] >= line.size())
			continue;
		...
		while (secondNodeStart < path.size() && path[secondNodeStart] != '>' && path[secondNodeStart] != '<')
			secondNodeStart += 1;
...
void alignReads(AlignerParams params)
{
	...
	struct timeval start_gfa_time, end_gfa_time;
	struct timeval start_seed_time, end_seed_time;

	gettimeofday(&start_gfa_time, NULL);
	...
	if (params.useDiploidHeuristic)
	{
		...
		if (params.diploidHeuristicCacheFile != "")
		{
			...
			if (file.good())
			{
				...
				{
					if (foundk[i] != params.diploidHeuristicK[i])
						valid = false;
				}
				if (!valid)
				{
	...
	gettimeofday(&end_gfa_time, NULL);
	read_gfa_time += (double)(end_gfa_time.tv_sec - start_gfa_time.tv_sec) +
					 (double)(end_gfa_time.tv_usec - start_gfa_time.tv_usec) / 1e6;

	gettimeofday(&start_seed_time, NULL);
	...
	Seeder seeder{params, seedHitsToThreads, memseeder, minimizerseeder};
	gettimeofday(&end_seed_time, NULL);
	seed_time += (double)(end_seed_time.tv_sec - start_seed_time.tv_sec) +
				 (double)(end_seed_time.tv_usec - start_seed_time.tv_usec) / 1e6;

	std::cout << "==== ONLY FOR TEST, NO EXTEND ====" << std::endl;
#ifdef RUN_MODE_ALIGNMENT
	...
	case Seeder::Mode::Mum:
		...
		if (seeder.mumCount != std::numeric_limits<size_t>::max())
			std::cout << ", max count " << seeder.mumCount;
		std::cout << std::endl;
		break;
	case Seeder::Mode::Mem:
		std::cout << "MEM seeds, min length " << seeder.mxmLength;
		if (seeder.memCount != std::numeric_limits<size_t>::max())
			std::cout << ", max count " << seeder.memCount;
		std::cout << std::endl;
		break;
	...
	if (seeder.mode != Seeder::Mode::None)
		std::cout << "Seed cluster size " << params.seedClusterMinSize << std::endl;
	if (seeder.mode != Seeder::Mode::None && params.maxClusterExtend != std::numeric_limits<size_t>::max())
		std::cout << "Extend up to " << params.maxClusterExtend << " seed clusters" << std::endl;
	if (seeder.mode != Seeder::Mode::None && params.maxClusterExtend == std::numeric_limits<size_t>::max())
		std::cout << "Extend all seed clusters" << std::endl;

	std::cout << "Alignment bandwidth " << params.alignmentBandwidth;
	if (params.maxCellsPerSlice != std::numeric_limits<size_t>::max())
		std::cout << ", tangle effort " << params.maxCellsPerSlice;
	std::cout << std::endl;

	if (params.selectionECutoff != -1)
		std::cout << "Discard alignments with an E-value > " << params.selectionECutoff << std::endl;
	...
	if (params.maxTraceCount != std::numeric_limits<size_t>::max())
		std::cout << "Backtrace from " << params.maxTraceCount << " highest scoring local maxima per cluster" << std::endl;

	if (params.outputGAMFile != "")
		std::cout << "write alignments to " << params.outputGAMFile << std::endl;
	if (params.outputJSONFile != "")
		std::cout << "write alignments to " << params.outputJSONFile << std::endl;
	if (params.outputGAFFile != "")
		std::cout << "write alignments to " << params.outputGAFFile << std::endl;
	if (params.outputCorrectedFile != "")
		std::cout << "write corrected reads to " << params.outputCorrectedFile << std::endl;
	if (params.outputCorrectedClippedFile != "")
		std::cout << "write corrected & clipped reads to " << params.outputCorrectedClippedFile << std::endl;
	...
	std::thread fastqThread{[files = params.fastqFiles, &readFastqsQueue, &readStreamingFinished]()
							{ readFastqs(files, readFastqsQueue, readStreamingFinished); }};
	std::thread GAMwriterThread{[file = params.outputGAMFile, &outputGAM, &deallocAlns, &allThreadsDone, &GAMWriteDone, verboseMode = params.verboseMode]()
								{ if (file != "") consumeBytesAndWrite(file, outputGAM, deallocAlns, allThreadsDone, GAMWriteDone, verboseMode, false); else GAMWriteDone = true; }};
	std::thread GAFwriterThread{[file = params.outputGAFFile, &outputGAF, &deallocAlns, &allThreadsDone, &GAFWriteDone, verboseMode = params.verboseMode]()
								{ if (file != "") consumeBytesAndWrite(file, outputGAF, deallocAlns, allThreadsDone, GAFWriteDone, verboseMode, true); else GAFWriteDone = true; }};
	std::thread JSONwriterThread{[file = params.outputJSONFile, &outputJSON, &deallocAlns, &allThreadsDone, &JSONWriteDone, verboseMode = params.verboseMode]()
								 { if (file != "") consumeBytesAndWrite(file, outputJSON, deallocAlns, allThreadsDone, JSONWriteDone, verboseMode, true); else JSONWriteDone = true; }};
	std::thread correctedWriterThread{[file = params.outputCorrectedFile, &outputCorrected, &deallocAlns, &allThreadsDone, &correctedWriteDone, verboseMode = params.verboseMode, uncompressed = !params.compressCorrected]()
									  { if (file != "") consumeBytesAndWrite(file, outputCorrected, deallocAlns, allThreadsDone, correctedWriteDone, verboseMode, uncompressed); else correctedWriteDone = true; }};
	std::thread correctedClippedWriterThread{[file = params.outputCorrectedClippedFile, &outputCorrectedClipped, &deallocAlns, &allThreadsDone, &correctedClippedWriteDone, verboseMode = params.verboseMode, uncompressed = !params.compressClipped]()
											 { if (file != "") consumeBytesAndWrite(file, outputCorrectedClipped, deallocAlns, allThreadsDone, correctedClippedWriteDone, verboseMode, uncompressed); else correctedClippedWriteDone = true; }};

	for (size_t i = 0; i < params.numThreads; i++)
	{
		threads.emplace_back([&alignmentGraph, &readFastqsQueue, &readStreamingFinished, i, seeder, params, &outputGAM, &outputJSON, &outputGAF, &outputCorrected, &outputCorrectedClipped, &deallocAlns, &stats, &diploidHeuristic]()
							 { runComponentMappings(alignmentGraph, diploidHeuristic, readFastqsQueue, readStreamingFinished, i, seeder, params, outputGAM, outputJSON, outputGAF, outputCorrected, outputCorrectedClipped, deallocAlns, stats); });
	}
	...
	if (stats.allAlignmentsCount > stats.alignments)
		std::cout << " (" << (stats.allAlignmentsCount - stats.alignments) << " additional alignments discarded)";
	std::cout << std::endl;
